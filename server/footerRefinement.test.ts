...
describe("footer visual refinement", () => {
  ...
  it("includes a restrained linked development credit in the footer", async () => {
    ...
    expect(page).toContain('footer-disclosure footer-explore ${panelOpen("explore") ? "is-open" : ""}');
    expect(page).toContain('footer-disclosure footer-connect ${panelOpen("connect") ? "is-open" : ""}');
    expect(page).toContain('const [footerPanels, setFooterPanels] = useState({ explore: false, connect: false })');
    expect(page).toContain('window.matchMedia("(max-width: 640px)")');
    expect(page).toContain('className="footer-contact-compact"');
    expect(styles).toContain("@media (max-width:640px)");
    expect(styles).toContain("grid-template-rows:auto 0fr");
    expect(styles).toContain("grid-template-rows:auto 1fr");
    expect(styles).toContain("transition:grid-template-rows .24s var(--ease)");
