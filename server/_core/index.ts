...
export async function createApp(server?: ReturnType<typeof createServer>): Promise<Express> {
  ...
  if (process.env.NODE_ENV === "development") {
    if (!server) throw new Error("A HTTP server is required for Vite development mode.");
...
async function startServer() {
  ...
  // Configure app routes against the HTTP server used by Vite HMR in development.
