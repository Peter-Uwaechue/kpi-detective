...
export default function KPIDetective() {
  ...
  if (stage === "results" && analysis && dataset) return <main className={`kpi-app ${dark ? "dark-mode" : ""}`}>
    ...
    <div className="dashboard-shell">
      <aside className="dashboard-sidebar"><div className="sidebar-title">Analysis</div><button className="sidebar-link active"><PanelTop size={17} />Overview</button><button className="sidebar-link" onClick={() => setShowData(true)}><TableProperties size={17} />Cleaned data</button><div className="sidebar-bottom"><div className="sidebar-title">Recent</div>{history.slice(0, 3).map(item => <button className="history-item" key={item.id} title={item.summary}><History size={14} /><span>{item.metric}<small>{new Date(item.at).toLocaleDateString(undefined, { month: "short", day: "numeric" })}</small></span><b className={item.changePercent >= 0 ? "positive" : "negative"}>{changeText(item.changePercent)}</b></button>)}<Link href="/" className="back-site"><ArrowLeft size={15} />Willers Solutions</Link></div></aside>
  ...
  return <main className={`kpi-app landing ${dark ? "dark-mode" : ""}`}>
    <header className="app-header"><button className="brand-button" onClick={() => { setStage("upload"); setDataset(null); }}><Logo /></button><div className="header-actions"><Link href="/" className="site-return"><ArrowLeft size={15} />Willers Solutions</Link><button className="theme-control" onClick={() => setDark(current => !current)}><Moon size={16} />{dark ? "Light" : "Dark"}</button></div></header>
