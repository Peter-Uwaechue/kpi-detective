...
type ContainmentReviewProposal = { id: string; column: string; containedValue: string; containingValue: string; containedCount: number; containingCount: number; status: "pending" | "merged" | "kept-separate" | "superseded" };
...
export default function KPIDetective() {
  ...
  const decideRemoteContainment = async (proposalId: string, decision: "merge" | "keep-separate") => {
    if (!remoteImportId || reviewContainment.isPending) return;
    try {
      setError("");
