...
type PossibleAliasProposal = { id: string; column: string; abbreviation: string; expansion: string; abbreviationCount: number; expansionCount: number; combinedCount: number; dominantValue: string; dominantCount: number };
type RemoteCleaningLogEntry = { key: string; title: string; detail: string; count: number; severity: "success" | "warning" | "info"; proposals?: PossibleAliasProposal[] };
...
function PossibleAliasReview({ proposals, onDecision, pending }: { proposals: PossibleAliasProposal[]; onDecision: (proposalId: string, decision: "merge" | "keep-separate") => void; pending: boolean }) {
  if (!proposals.length) return null;
  return <section className="possible-alias-review" aria-labelledby="possible-alias-heading"><div className="possible-alias-heading"><div><span className="eyebrow">Your decision required</span><h2 id="possible-alias-heading">Possible abbreviation aliases</h2></div><span>{proposals.length} pending</span></div><p>These values are textually different and were <strong>not merged automatically</strong>. Their low combined frequency is only a prompt to check your business meaning.</p><div className="possible-alias-list">{proposals.map(proposal => <article className="possible-alias-card" key={proposal.id}><div><span className="cause-label">{proposal.column}</span><h3>Could <b>{proposal.abbreviation}</b> mean <b>{proposal.expansion}</b>?</h3><p><b>{proposal.abbreviation}</b> appears {proposal.abbreviationCount.toLocaleString()} time{proposal.abbreviationCount === 1 ? "" : "s"}; <b>{proposal.expansion}</b> appears {proposal.expansionCount.toLocaleString()} time{proposal.expansionCount === 1 ? "" : "s"}. They remain separate until you approve a merge.</p></div><div className="possible-alias-actions"><button className="secondary-action" onClick={() => onDecision(proposal.id, "keep-separate")} disabled={pending}>No, keep separate</button><button className="primary-action" onClick={() => onDecision(proposal.id, "merge")} disabled={pending}>{pending ? "Saving…" : "Yes, merge them"}</button></div></article>)}</div></section>;
}
...
export default function KPIDetective() {
  ...
  const reviewPossibleAlias = trpc.kpiImports.reviewPossibleAlias.useMutation();
  ...
  const remotePossibleAliases = remoteImport ? remoteLogs(remoteImport.cleaningSummaryJson).flatMap(log => log.key === "possible-aliases" ? log.proposals ?? [] : []) : [];
  ...
  const decideRemotePossibleAlias = async (proposalId: string, decision: "merge" | "keep-separate") => {
    if (!remoteImportId || reviewPossibleAlias.isPending) return;
    try {
      setError("");
      const result = await reviewPossibleAlias.mutateAsync({ importId: remoteImportId, proposalId, decision });
      setAnalysis(result.analysis as KpiAnalysis);
      await remoteStatus.refetch();
    } catch (reason) { setError(reason instanceof Error ? reason.message : "We could not save that alias decision."); }
  };
  ...
  const driverDirection = analysis?.change === undefined || analysis.change === 0 ? "moving the KPI" : analysis.change < 0 ? "driving the decline" : "driving the growth";
  const offsetDirection = analysis?.change !== undefined && analysis.change < 0 ? "Positive factors offsetting the decline" : "Negative factors offsetting the growth";
